function ttlem_out = ttlem_WD_Geology(DEM,U,p,ThreeDGEOL,Mask4,FD,varargin)
...
% show the erodibility
hGUI1  = preparegui(p.K_weight);
 set(hGUI1.ax.time,'xlim',[0 tspan]);
...
ThreeDGEOL(:)=ThreeDGEOL(:)-(0.0001*dt*nriter);
...
while iter < nriter && ~get(hGUI.stopbutton,'value')
    ...
    ThreeDGEOL=ThreeDGEOL+0.0001.*dt;
   ...
   %% Update K values
    ...
    p.K_weight.Z(H1.Z<ThreeDGEOL & H1.Z>ThreeDGEOL-300)=1e-9;

    p.K_weight.Z(H1.Z<ThreeDGEOL-300)=6e-6;

   % p.K_weight.Z(H1.Z<ThreeDGEOL & (H1.Z & Mask4))=7e-6;

    p.Kw = k_init.*p.K_weight.Z;
    ...
    % Show the erobibility
    if mod(iter,p.ploteach)==0
        tspanstr = num2str(tspan);
        set(hGUI1.hIm,'Cdata',p.K_weight.Z);
        set(hGUI1.timedisp,'String',[num2str(round(t)) ' / ' tspanstr ' years (' num2str(t/tspan*100,'%3.1f') '%)'] );
        
        clim  = get(hGUI1.ax.main,'clim');
        tt    = get(hGUI1.tsmin,'Xdata');
        ydmin = get(hGUI1.tsmin,'Ydata');
        ydmax = get(hGUI1.tsmax,'Ydata');
        ydmean= get(hGUI1.tsmean,'Ydata');
        tt    = [tt(:); t];
        set(hGUI1.tsmin,'XData',tt,'YData',[ydmin(:);clim(1)]);
        set(hGUI1.tsmax,'XData',tt,'YData',[ydmax(:);clim(2)]);
        set(hGUI1.tsmean,'XData',tt,'YData',[ydmean(:);mean(p.K_weight.Z(~isnan(p.K_weight.Z)))]);
